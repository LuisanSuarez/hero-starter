...
var moves = {
  ...
  aggressor: function (gameData, helpers) {
    ...
    console.log({
      ...
      team0: gameData.teams[0],
      team1: gameData.teams[1],
      mines: gameData.diamondMines,
      well: gameData.healthWells,
    });
  ...
  healthNut: function (gameData, helpers) {
    ...
    if (gameData.activeHero.health <= 70) {
      // If it is, head towards the nearest health well
      return helpers.findNearestHealthWell(gameData);
    } else {
  ...
  earlyMinerLateAgressor: function (gameData, helpers) {
    var myHero = gameData.activeHero;
    var choices = ["North", "South", "East", "West"];
...
var move = moves.earlyMinerLateAgressor;
